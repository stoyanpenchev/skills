...
Emit inbound findings with the same FINDING block and a `bug_class` from the vocabulary under Output, re-anchored to the in-scope function that holds the invariant — `assumption:` is the invariant, `external_ref:` is the external mutator that breaks it.
...
```
FINDING | contract: <IN-SCOPE contract> | function: <IN-SCOPE function> | bug_class: integration-<defect> | group_key: <IN-SCOPE Contract> | <IN-SCOPE function> | integration-<defect>
...
The `bug_class` segment is one of these labels. One label for every seam bug is one ledger record for every seam bug, and memory recognises none of them again:

| Label | When |
|---|---|
| `integration-unguarded-mutator` | inbound — an external mutator whose guard is NONE |
| `integration-guard-asymmetry` | inbound — a pair of comparable operations whose guards differ |
| `integration-ratio-decoupling` | inbound — one side of a coupled ratio moves without the other |
| `integration-stale-oracle` | outbound — a read that is stale or manipulable |
| `integration-return-mismatch` | outbound — a decimals, unit, zero or truncated return |
| `integration-revert-mismatch` | outbound — reverts where success was assumed, or succeeds where a revert was |
| `integration-token-behavior` | outbound — a fee-on-transfer or rebasing token |

Invent `integration-<defect>` only when none of these is the same class of bug. A label already listed for this contract and function in `known-findings.md` wins over every one of them — the repository's own word, always.
